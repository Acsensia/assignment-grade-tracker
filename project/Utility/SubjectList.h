...
#include "Subject.h"
namespace PROJECT {
	class SubjectList : public List<Subject> {
	public:
		using List::List;

		enum SubjectVariable {
			name,
			uniqueNumber,////
			taskCount,
			tasks_name,
		...
		int findByValue(string stringReference, SubjectVariable referencedVariable) {
			...
			case taskCount:
			{
				int intReference = stoi(stringReference);
				for (; i > -1; i--) {
					if (currentlySelected->data.taskCount == intReference) break;
					currentlySelected = currentlySelected->prev;
				}
			}
			break;
			...
			{
				for (; i > -1; i--) {
					if (currentlySelected->data.tasks.findByValue(stringReference, TaskList::name) >= 0) break;
					currentlySelected = currentlySelected->prev;
				}
			}
			break;
			case tasks_uniqueNumber:
			{
				for (; i > -1; i--) {
					if (currentlySelected->data.tasks.findByValue(stringReference, TaskList::uniqueNumber) >= 0) break;
					currentlySelected = currentlySelected->prev;
				}
			}
			break;
		...
		tuple<bool, string> toFile(const string& fileName, ofstream& out) {
			...
			try {
				for (int i = 0; i < this->length(); i++) {
					...
					out.write(reinterpret_cast<char*>(&currentlySelected->data.taskCount), sizeof(int));
					currentlySelected->data.tasks.toFile(fileName, out);
			...
			try {
				...
				auto fromFile_ItemReadingCycle = [&]() {
					try {////
						...
						in.read(reinterpret_cast<char*>(&data.taskCount), sizeof(int));
						this->add(data);
						this->getReference(-1)->tasks.fromFile(fileName, in, data.taskCount);
						return true;////
					}
